...
var getCode = function(opts, type) {
    ...
    if (type === 'custom') {
        ...
        opts.template.class = opts.template.class || fs.readFileSync(templates + (type === 'typescript-def' ? 'typescript-definitions-only.mustache' : type + '-class.mustache'), 'utf-8');
        opts.template.method = opts.template.method || fs.readFileSync(templates + (type === 'typescript' || type === 'typescript-def' ? 'typescript-' : '') + 'method.mustache', 'utf-8');
        if(type === 'typescript' || type === 'typescript-def') {
...
exports.CodeGen = {
    ...
    getTypescriptDefinitions: function(opts){
        if (opts.swagger.swagger !== '2.0') {
            throw 'Typescript is only supported for Swagger 2.0 specs.';
        }
        return getCode(opts, 'typescript-def');
    },
